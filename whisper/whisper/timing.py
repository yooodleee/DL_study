...
def add_word_timestapms(
    ...
    text_tokens_per_segment = [
        [token for token in segment["token"] if token < tokenizer.eot]
        for segment in segments
    ]

    text_tokens = list(itertools.chain.from_iterable(text_tokens_per_segment))
    alignment = find_alignment(model,
                               tokenizer,
                               text_tokens,
                               mel,
                               num_frames,
    ...
    median_duration = np.median(word_durations) \
                        if len(word_durations) > 0 else 0.0
    median_duration = min(0.7, float(median_duration))
    max_duration = median_duration * 2

    # hack: truncate long words at sentence boundaries.
    # a better segmentation algorithm based on VAD should be able to 
    # replace this.
    if len(word_durations) > 0:
        sentence_end_marks = ".。!！?？"
        # ensure words at sentence boundaries are not longer than twice 
        # the median word duration.
    ...
    for segment, text_tokens in zip(segments,
                                    text_tokens_per_segment):
        saved_tokens = 0
        words = []
        ...
        if len(words) > 0:
            ...
            if words[0]["end"] - last_speech_timestamp > median_duration * 4 \
                and (words[0]["end"] - words[0]["start"] > max_duration
                or (
                    len(words) > 1
            ...
            # prefer the segment-level start timestamp 
            ...
            if (
                ...
                words[0]["start"] = max(
                    0, 
                    min(words[0]["end"] - median_duration, segment["start"])
                )
            else:
