...
def transcribe(
    ...
    with tqdm.tqdm(total=content_frames,
        ...
        return dict(
            text=tokensizer.decode(all_tokens[len(initial_prompt_tokens) :]),
...
def cli():
    ...
    def valid_model_name(name):
        if name in available_models() or os.path.exists(name):
            return name
        raise ValueError(
            f"model should be one of {available_models()} or 
    ...
    parser.add_argument("--model", 
                        default="turbo", 
                        type=valid_model_name, 
                        help="name of the Whisper model to use")
    parser.add_argument("--model_dir", 
                        type=str, 
                        default=None, 
                        help="the path to save model files; \
                            uses ~/.cache/whisper by default")
    ...
    parser.add_argument("--output_format", 
                        ...
                        help="format of the output file; if not specified, \
                            all available formats will be produced")
    parser.add_argument("--verbose", 
                        type=str2bool, 
                        default=True, 
                        help="whether to print out the progress and \
                            debug messages")
    ...
    parser.add_argument("--task", 
                        ...
                        help="whether to perform X->X speech \
                            recognition ('transcribe') or X->English \
    ...
    parser.add_argument("--language", 
                        ...
                        help="language spoken in the audio, \
                            specify None to perform language detection")
    ...
    parser.add_argument("--best_of", 
                        type=optional_int, 
                        default=5, 
                        help="number of candidates when sampling with non-zero \
                            temperature")
    parser.add_argument("--beam_size", 
                        type=optional_int, 
                        default=5, 
                        help="number of beams in beam search, only applicable \
    ...
    parser.add_argument("--patience", 
                        ...
                        help="optional patience value to use in beam decoding, \
                            ...
                            the default (1.0) is equivalent to conventional \
                            beam search")
    parser.add_argument("--length_penalty", 
                        type=float, 
                        default=None, 
                        help="optional token length penalty coefficient (alpha) \
                            as in https://arxiv.org/abs/1609.08144, \
                            uses simple length normalization by default")

    parser.add_argument("--suppress_tokens", 
                        type=str, 
                        default="-1", 
                        help="comma-separated list of token ids to suppress \
                            during sampling; '-1' will suppress most special \
    ...
    parser.add_argument("--carry_initial_prompt", 
                        type=str2bool, 
                        default=False, 
                        help="if True, prepend initial_prompt to every \
                            internal decode() call. May reduce the \
                            effectiveness of condition_on_previous_text")

    parser.add_argument("--condition_on_previous_text", 
                        type=str2bool, 
                        default=True, 
                        help="if True, provide the previous output of the \
                            model as a prompt for the next window; \
                            disabling may make the text inconsistent across \
                            windows, but the model becomes less prone to \
    ...
    parser.add_argument("--temperature_increment_on_fallback", 
                        type=optional_float, 
                        default=0.2, 
                        help="temperature to increase when falling back \
                            when the decoding fails to meet either of the \
                            thresholds below")
    parser.add_argument("--compression_ratio_threshold", 
                        type=optional_float, 
                        default=2.4, 
                        help="if the gzip compression ratio is higher \
                            than this value, treat the decoding as failed")
    parser.add_argument("--logprob_threshold", 
                        type=optional_float, 
                        default=-1.0, 
                        help="if the average log probability is lower than \
                            this value, treat the decoding as failed")
    parser.add_argument("--no_speech_threshold", 
                        type=optional_float, 
                        default=0.6, 
                        help="if the probability of the <|nospeech|> token \
                            is higher than this value AND the decoding has \
                            failed due to `logprob_threshold`, \
                            consider the segment as silence")
    parser.add_argument("--word_timestamps", 
                        type=str2bool, 
                        default=False, 
                        help="(experimental) extract word-level timestamps \
                            and refine the results based on them")
    parser.add_argument("--prepend_punctuations", 
                        type=str, 
                        default="\"\'“¿([{-", help="if word_timestamps is \
                            True, merge these punctuation symbols with the \
                            next word")
    parser.add_argument("--append_punctuations", 
                        type=str, 
                        default="\"\'.。,，!！?？:：”)]}、", 
                        help="if word_timestamps is True, merge these \
                            punctuation symbols with the previous word")
    parser.add_argument("--highlight_words", 
                        type=str2bool, 
                        default=False, 
                        help="(requires --word_timestamps True) underline each \
                            word as it is spoken in srt and vtt")
    parser.add_argument("--max_line_width", 
                        type=optional_int, 
                        default=None, 
                        help="(requires --word_timestamps True) the maximum \
                            number of characters in a line before breaking the \
                            line")
    parser.add_argument("--max_line_count", 
                        type=optional_int, 
                        default=None, 
                        help="(requires --word_timestamps True) the maximum \
                            number of lines in a segment")
    parser.add_argument("--max_words_per_line", 
                        type=optional_int, 
                        default=None, 
                        help="(requires --word_timestamps True, no effect with \
                            --max_line_width) the maximum number of words \
                            in a segment")
    parser.add_argument("--threads", 
                        type=optional_int, 
                        default=0, 
                        help="number of threads used by torch for \
                            CPU inference; supercedes \
                            MKL_NUM_THREADS/OMP_NUM_THREADS")
    parser.add_argument("--clip_timestamps", 
                        type=str, 
                        default="0", 
                        help="comma-separated list start,end,start,end,... \
                            timestamps (in seconds) of clips to process, \
                            where the last end timestamp defaults to the end of \
                            the file")
    parser.add_argument("--hallucination_silence_threshold", 
                        type=optional_float, 
                        help="(requires --word_timestamps True) skip silent \
                            periods longer than this threshold (in seconds) \
