...
class BeamSearchDecoder(TokenDecoder):
    def __init__(
        ...
        assert (self.max_candidates > 0), f"Invalud beam size
        ({beam_size}) or patience ({patience})"
    ...
    def update(
        ...
        if tokens.shape[0] % self.beam_size != 0:
            raise ValueError(
                f"{tokens.shape}[0] % {self.beam_size} != 0")
        ...
        for i in range(n_audio):
            ...
            for sequence in sorted(scores, key=scores.get, reverse=True):
                if sequence[-1] == self.eot:
                    finished[sequence] = sources[sequence]
                else:
        ...
        complted = all(len(sequences) >= self.max_candidates
                       for sequences in self.finished_sequences)
        return tokens, complted
    
    def fianlize(
            self,
            preceding_tokens: Tensor,
            sum_logprobs: Tensor):
...
class ApplyTimestampRules(LogitFilter):
    ...
    def apply(self, logits: Tensor, tokens: Tensor):
        ...
        # timestamps have to appear in paris, except directly before EOT;
        ...
        for k in range(tokens.shape[0]):
            ...
            if timestamps.numel() > 0:
                ...
                # forbid timestamp tokens smaller than the last
                # also force each segment to have a nonzero length, to prevent
                # infinite looping
        ...
        if tokens.shape[1] == self.sample_begin:
            ...
            if self.max_initial_timestamp_index is not None:
                last_allowed = (self.tokenizer.timestamp_begin
        ...
        for k in range(tokens.shape[0]):
            timestamp_logprob = \
                logprobs[k, self.tokenizer.timestamp_begin :].logsumexp(dim=-1)
            
            max_text_token_logprob = \
                logprobs[k, : self.tokenizer.timestamp_begin].max()
...
class DecodingTask:
        ...
        try:
            for i in range(self.sample_len):
                ...
                if (i == 0 and self.tokenizer.no_speech is not None):
                    probs_at_sot = \
                        logits[:, self.sot_index].float().softmax(dim=-1)
                    no_speech_probs = \
                        probs_at_sot[:, self.tokenizer.no_speech].tolist()
                
                # now need to consider the logits at the last token knly
                logits = logits[:, -1]

                # apply the logit filters, 
                # e.g. for suppressing or applying penalty to
    ...
    def run(self, mel: Tensor) -> List[DecodingResult]:
        ...
        tokens: Tensor = \
            torch.tensor([self.initial_tokens]).repeat(n_audio, 1)
        
        # detect language if requested, overwrting the language token
        languages, language_probs = self._detect_language(audio_features, 
                                                            tokens)
        if self.options.task == "lang_id":
            return [
                DecodingResult(audio_features=features,
                    ...
                    for features, language, probs in zip(audio_features,
                                                        languages,
                                                        language_probs)]
        
        # repeat text tensors by the group size, 
        ...
        tokens, sum_logprobs, no_speech_probs = \
            self._main_loop(audio_features, tokens)
        
        # reshape the tensors to have (n_audio, n_group) 
        ...
        tokens: List[List[Tensor]] = [
            [t[self.sample_begin : (t == tokenizer.eot).nonzero()[0, 0]] 
            for t in s] for s in tokens
        ]
        ...
        avg_logprobs: List[float] = [lp / (len(t) + 1) 
                                    for t, lp in zip(tokens, sum_logprobs)]
        ...
        return [
            ...
            for text, language, tokens, features, \
                avg_logprob, no_speech_prob in zip(*fields)]
    ...
    def decode(
        ...
        Parameters:
            ...
            mel(torch, shape = (80, 3000) or (*, 80, 3000))
                A tensor containing the Moel spectrogram(s)
            Options(DecodingOptions)
                A dataclass that contains all necessary options for decoding
                    30-second segments

        Returns:
            result(Union[Decodingresult, List[DecodingResult]])
                The result(s) of decoding contained in 'DecodingResult' 
                    dataclass instance(s).
        """
